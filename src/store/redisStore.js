...
function makeRedisStore(url, logger, groupCache) {
  ...
  return {
    ...
    async clearAiTurns(chatJid) {
      try { await client.del(`ai_conv:${chatJid}`); return true; }
    ...
    async recordAiUsage({ day, provider, model, promptTokens = 0, completionTokens = 0, costUsd = 0 }) {
      try {
        ...
        const cur  = prev ? (() => { try { return JSON.parse(prev); } catch { return {}; } })() : {};
        const next = {
    ...
    async getAiUsageDayTotal(day) {
      try {
        ...
        for (const v of Object.values(all || {})) {
          try { total += Number(JSON.parse(v).cost_usd || 0); } catch { /* ignore */ }
        }
        return total;
    ...
    async getAiUsageByDay(limit = 30) {
      try {
        ...
        for (const k of dayKeys) {
          const d = k.split(':')[1];
          if (!rolled.has(d)) rolled.set(d, { day: d, cost_usd: 0, prompt_tokens: 0, completion_tokens: 0, calls: 0 });
    ...
    async setAiChatOptIn(chatJid, { enabled = false, persona = null, provider = null, model = null } = {}) {
      try {
        await client.set(`ai_opt_in:${chatJid}`, JSON.stringify({
          enabled: !!enabled,
          persona,
          provider,
          model,
    ...
    async incrAiRateUser(userJid, hourBucket) {
      try {
        ...
        try { await client.expire(key, ttl); } catch { /* ignore */ }
        return Number(n);
