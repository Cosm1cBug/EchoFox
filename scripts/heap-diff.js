...
function fail(msg, code = 1) {
  console.error(`[heap-diff] ERROR: ${msg}`);
  process.exit(code);
}
...
console.log(
  `${'CLASS'.padEnd(45)}  ${'+Δ count'.padStart(10)}  ${'+Δ size'.padStart(14)}  ${'pct'.padStart(7)}`,
);
...
for (const d of deltas.slice(0, 30)) {
  ...
  const sizeStr =
    d.deltaSize >= 0
      ...
      : `${(d.deltaSize / 1024).toFixed(1)} KB`;
  console.log(
    `${d.key.slice(0, 45).padEnd(45)}  ${String(d.deltaCount).padStart(10)}  ${sizeStr.padStart(14)}  ${pct.padStart(7)}`,
  );
}
