      ...
      for (const target of repos) {
        ...
        if (kind === 'releases' || kind === 'both') {
          for (const r of await fetchReleases(owner, repo)) {
            ...
            try {
              await sock.sendMessage(jid, { text: formatRelease(owner, repo, r) });
            } catch (err) {
              logger.warn({ jid, err: err.message }, 'send release failed');
              continue;
            }
        ...
        if (kind === 'advisories' || kind === 'both') {
          for (const a of await fetchAdvisories(owner, repo)) {
            ...
            try {
              await sock.sendMessage(jid, { text: formatAdvisory(owner, repo, a) });
            } catch (err) {
              logger.warn({ jid, err: err.message }, 'send advisory failed');
              continue;
            }
