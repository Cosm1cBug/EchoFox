...
(async () => {
  ...
  if (!ctx || Object.values(ctx).some((v) => v == null)) {
    logger.fatal({ type, ctx: Object.keys(ctx || {}) },
  ...
  try {
    if (statusOnly) {
      ...
      logger.info('NOTE: --status currently logs migration status from the runner; it does NOT prevent application. Cancel with Ctrl-C if you only want to inspect.');
    }
